...
public class GameLogic {
    public static void playSeaBattle() {
        // Имитация начального экрана с запросом имен игроков
        System.out.println("\n                                                                                 SEA BATTLE v.0.1\n");
        ...
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            // Имитация экрана с правилами игры
            System.out.println("\n\n\n\n");
            ...
            System.out.println("                   Вражеское поле: на этом поле отмечаются подбитые вражеские корабли, а также отмечены точки знаком " + FieldCell.SHIP_HALO + " по координатам которых вы стреляли, но выстрел не достиг цели.\n");
    ...
    // Метод имитации экрана при расстановке кораблей
    protected static void setShip(Player player, Scanner scanner, Integer ship) {
        while (true) {
            System.out.println("\n\n\n\n\n\n\n\n\n\n\n\n                                                                            Расстановка флота игрока " + player.getName() + "\n\n\n\n");
            printMyField(player);
            if (ship == 4) {
                System.out.println("\n\n\n\n\n\n\n\n                                                   Введите координаты для ЧЕТЫРЕХПАЛУБНОГО корбаля (формат x y,x y,x y,x y): \n\n");
            }
            if (ship == 3) {
                System.out.print("\n\n\n\n\n\n\n\n                                                   Введите координаты для ТРЕХПАЛУБНОГО корбаля (формат x y,x y,x y): \n\n");
            }
            if (ship == 2) {
                System.out.print("\n\n\n\n\n\n\n\n                                                        Введите координаты для ДВУХПАЛУБНОГО корбаля (формат x y,x y): \n\n");
            }
            if (ship == 1) {
                System.out.print("\n\n\n\n\n\n\n\n                                                          Введите координаты для ОДНОПАЛУБНОГО корбаля (формат x y): \n\n");
            }
            System.out.print("                                                                                        ");
            ...
            try {
                ...
                if (count == 2 && player.getMyField()[coordinatesInt[0]][coordinatesInt[1]] != FieldCell.SHIP && player.getMyField()[coordinatesInt[0]][coordinatesInt[1]] != FieldCell.SHIP_HALO) {
                    ...
                    shipHalo(coordinatesInt,player);
                    break;
                } else if (count == 2) {
    ...
    protected static void shipHalo(int[] coordinatesInt, Player player) {
        int x;
        int y;
        for (int i = 1; i <coordinatesInt.length; i += 2) {
            ...
            y = coordinatesInt[i];
            if (x == 0 && y != 0 && y != 9) {
                if (player.getMyField()[x][y - 1] == FieldCell.EMPTY) {
                    player.getMyField()[x][y - 1] = FieldCell.SHIP_HALO;
                }
                if (player.getMyField()[x + 1][y - 1] == FieldCell.EMPTY) {
                    player.getMyField()[x + 1][y - 1] = FieldCell.SHIP_HALO;
                }
                if (player.getMyField()[x + 1][y] == FieldCell.EMPTY) {
                    player.getMyField()[x + 1][y] = FieldCell.SHIP_HALO;
                }
                if (player.getMyField()[x + 1][y + 1] == FieldCell.EMPTY) {
                    player.getMyField()[x + 1][y + 1] = FieldCell.SHIP_HALO;
                }
